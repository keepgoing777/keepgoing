...
SELECT * FROM DEPARTMENT; // DEPT_ID, DEPT_TITLE, LOCATION_ID(L1,L2,L3) 
...
SELECT * FROM USER_INFO; -- USER_ID
SELECT * FROM REVIEW; -- REVIEW_ID, MOVIE_ID, USER_ID, RATING, REVIEW_DATE 
SELECT * FROM MOVIE; -- MOVIE_ID, TITLE, RELEASE_DATE, DIRECTOR
SELECT * FROM ACTOR; -- ACTOR_ID, NAME, BIRTHDATE, NATIONALITY
SELECT * FROM MOVIE_ACTOR; -- MOVIE_ID, ACTOR_ID, ROLE 
...
SELECT TITLE, AVG(RATING), DENSE_RANK() OVER(ORDER BY AVG(RATING) DESC) RANK
FROM REVIEW
JOIN MOVIE USING(MOVIE_ID)
GROUP BY TITLE
HAVING (AVG(RATING) > (SELECT AVG(RATING) FROM REVIEW)); -- FROM 절 서브쿼리
-- 영화제목, 평균평점, 평균평점 기준 랭크 